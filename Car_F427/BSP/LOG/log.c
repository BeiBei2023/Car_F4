...
#include <string.h>
#include "cmsis_os.h"
#include <locale.h> // 添加locale头文件

#define LOG_BUFFER_SIZE 1024                // 环形缓冲区大小
static uint8_t log_buffer[LOG_BUFFER_SIZE]; // 环形缓冲区
static volatile uint16_t log_head = 0;      // 写入位置
static volatile uint16_t log_tail = 0;      // 读取位置
...
void ProcessLog(void)
{
    ...
    {
        uint16_t send_len = (log_head > log_tail) ? (log_head - log_tail) : (LOG_BUFFER_SIZE - log_tail);

        // 检查DMA是否空闲
        if (HAL_UART_GetState(&huart6) == HAL_UART_STATE_READY)
        {
            // 使用DMA发送数据
            HAL_UART_Transmit_DMA(&huart6, &log_buffer[log_tail], send_len);
            log_tail = (log_tail + send_len) % LOG_BUFFER_SIZE;
        }
        else
        {
            // 如果DMA忙碌，改用阻塞模式发送
            HAL_UART_Transmit(&huart6, &log_buffer[log_tail], send_len, HAL_MAX_DELAY);
            log_tail = (log_tail + send_len) % LOG_BUFFER_SIZE;
        }
    }
...
void EM_LOG(const int level, const char *fun, const int line, const char *fmt, ...)
{
    ...
    if (level >= LOG_LEVEL && buf_len >= 0)
    {
        ...
        {
            uint16_t next_head = (log_head + 1) % LOG_BUFFER_SIZE;

            // 如果缓冲区满，等待空间释放
            while (next_head == log_tail)
            {
                ProcessLog(); // 主动处理日志，释放空间
                osDelay(1);   // 避免忙等待
            }

            log_buffer[log_head] = log_message[i];
            log_head = next_head;
        }
